...
void irobotNavigationStatechart(
	...
	static const double			a = 0.75;						/* Filter constant */
	static int32_t				distanceCheck = 50;				/* distance to travel beofore a check of incline */
	...
	/**************************************/
		...
		if (sensors.bumps_wheelDrops.bumpLeft
			...
			obstacles++;
			state = BACK;
		} else if (!avoid && (abs(netDistance - distanceAtManeuverStart) >= distanceCheck)) {
			if (filtered_z < 0.9 && (absDouble(filtered_x) > 0.13 ||
									 absDouble(filtered_y) > 0.13)) {
				orient = 1;
				state = TURN;
			}
		...
		if (abs(netAngle - angleAtManeuverStart) >= turnAngle) {
			if (orient) {
				if (absDouble(filtered_y) < 0.05 && filtered_x > 0.11) {
					orient = 0;
					distanceCheck = 150;
					angleAtManeuverStart = netAngle;
					distanceAtManeuverStart = netDistance;
