...
void Aio_Write(MyRio_Aio* channel, double value)
{
    ...
    if (channel->is_signed)
    {
         ...
         */
        value = value / channel->scale_weight - channel->scale_offset;
    ...
    {
        value = (value / channel->scale_weight - channel->scale_offset) + 0.5;
...
/**
 * Reads the scaling weight and offset for the channel.
 *
 * Reads the least significant bit weight for the channel. This represents the
 * change in voltage for every one bit change in the value. For example,
 * AnalogWrite(channel, 2) outputs AnalogScalingWeight(channel) * 2 (in volts).
 *
 * Reads the offset between the input/output value and the actual voltage.
 *
 * @param[in,out]  channel  A struct containing the registers for the AIO
...
void Aio_Scaling(MyRio_Aio* channel)
{
    ...
    status = NiFpga_ReadU32(myrio_session, channel->wght, &weight);

    /*
     * Get the value of the offset scaling register.
     *
     * The returned NiFpga_Status value is stored for error checking.
     */
    NiFpga_MergeStatus(&status, NiFpga_ReadU32(myrio_session, channel->ofst,
            &offset));
     ...
     */
    MyRio_ReturnIfNotSuccess(status,
            "Could not read from the AIO scaling registers!");

    /*
     * Convert from nano-volts to volts.
     */
    channel->scale_weight = weight / 1000000000.0;
    channel->scale_offset = offset / 1000000000.0;
}
