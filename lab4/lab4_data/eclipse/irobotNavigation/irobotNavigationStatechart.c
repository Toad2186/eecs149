...
void irobotNavigationStatechart(
	...
	static int32_t				rememberedTurns = 0;			/* how many times we turned to avoid obstacles */
...
//		}
	...
	else if(state == DRIVE && (sensors.bumps_wheelDrops.bumpLeft
...
//		rememberedTurns++;
	...
	else if((state == TURN || state == TURNRIGHT) && abs(netAngle - angleAtManeuverStart) >= 90){
		angleAtManeuverStart = netAngle;
		distanceAtManeuverStart = netDistance;
//		if(rememberedTurns){
//			state = CORRECT;
